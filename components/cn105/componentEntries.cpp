    ...
    switch (state_) {
        case DriverState::BOOT: {
            ...
            if (wifi::global_wifi_component != nullptr && !wifi::global_wifi_component->is_connected()) {
                ...
                ESP_LOGI(LOG_CONN_TAG, "Bootstrap connection: waiting for WiFi before UART/CONNECT init");
                return;
            }
#endif
            // WiFi ready (or no WiFi) — check grace delay
            this->transition_to_(DriverState::WAIT_GRACE);
            ESP_LOGI(LOG_CONN_TAG, "Bootstrap connection: grace period %ums for OTA logs", this->conn_bootstrap_delay_ms_);
            return;
        }
...
#endif
            this->transition_to_(DriverState::WAIT_GRACE);
            ESP_LOGI(LOG_CONN_TAG, "Bootstrap connection: WiFi connected, grace period %ums", this->conn_bootstrap_delay_ms_);
            return;
        }
        ...
        case DriverState::WAIT_GRACE: {
            ...
            ESP_LOGI(LOG_CONN_TAG, "Bootstrap connection: init UART + send CONNECT (loop)");
            this->setupUART();
            this->sendFirstConnectionPacket();
