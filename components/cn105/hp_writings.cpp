...
void CN105Climate::sendFirstConnectionPacket() {
    if (this->isUARTReady_()) {
        ...
        // CONNECT has a pre-calculated checksum in the constant; if command byte is modified, it must be recalculated.
        ...
        // Byte details in DEBUG on connection tag
        ...
        this->set_timeout("checkFirstConnection", 10000, [this]() {
            if (!this->isHeatpumpConnected()) {
                ...
                // Automatic fallback: if installer mode is requested but heat pump ignores 0x5B,
                // retry once in standard mode (0x5A) to preserve connectivity.
...
void CN105Climate::buildAndSendRequestsInfoPackets() {
    if (this->isHeatpumpConnected()) {
        ...
        this->nbCycles_++;
        // Send the first enabled request (list is registered once in constructor)
